export const isSearchDataSame = (
 ...
 var queryString = Object.keys(searchMainData).reduce(
  (init, currentValue, currentIndex) => {
   if (!searchMainData[currentValue]) return init;
   else
    return (
     "?" +
     `${currentValue}=${searchMainData[currentValue]}` +
     "&" +
     init.slice(1)
    );
  },
 ...
 if (Object.keys(queryObject).length === 0) {
  return history.push(
   location.pathname +
    (queryString ? queryString + "&pageNumber=1" : "?pageNumber=1")
  );
 }
 ...
 if (
  Object.keys(notUndefinedObject).length !== Object.keys(querySearchData).length
 ) {
  return history.push(
   location.pathname +
    (queryString ? queryString + "&pageNumber=1" : "?pageNumber=1")
