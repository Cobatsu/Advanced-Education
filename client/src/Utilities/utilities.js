...
export const studentListHelperPackage = id => {
 ...
 const tableInformations = item => {
  ...
  return [
   ...
   <Capsule style={{...studentStatusColor(registerState).style}}>
    {" "}
    {studentStatusColor(registerState).text}{" "}
   </Capsule>,
   item.owner?._id === id ? (
    <Capsule>{restrictWord(fullName, 16)}</Capsule>
   ) : (
    restrictWord(fullName, 16)
   ),
   item.registerDate,
  ];
 };
...
export const reportListHelperPackage = id => {
 ...
 const tableInformations = item => {
  ...
  return [
   ...
   item?.meetingDate,
   <Capsule style={{...statusColors(item).style}}>
    {" "}
    {statusColors(item).text}{" "}
   </Capsule>,
   !item.isContacted ? (
    "—"
...
export const personelListHelperPackage = currentUser => {
 ...
 const filterIconOptions = user => {
  ...
  if (role === "Admin") {
   PersonelOptions = PersonelOptions.filter(
    ({desc}) => desc !== "Raporlar" && desc !== "Öğrenciler"
   );
  }
 ...
 const pathGenerator = (item, id) => {
  if (item === "Raporlar" || item === "Bayiler") {
   var pageQuery = "?pageNumber=1";
  } else {
   var pageQuery = "";
  }

  return (
   "/home/personel_listesi/" +
  ...
  return {
   ...
   icon: (
    <img
     ...
     width='25'
     height='25'
     src='/animation_500_kbv6hrft.gif'
    />
   ),
