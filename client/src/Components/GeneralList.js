...
const GeneralList = ({
 ...
 const {filterIconOptions, pathGenerator, TopRows, tableInformations} =
  helperPackage;
 ...
 const SwitchRow = (Amount, ref) => event => {
  ...
  if (Amount == -50)
   for (let i = 0; i < refs.current.length; i++) {
    if (i !== refs.current.indexOf(ref) && refs.current[i].current) {
 ...
 return (
  <HiddenWrapper>
   ...
   >
    ( {subPagesCount || 0} ) Sonuç Bulundu{" "}
   </h1>

   {loading && !resetSubPage ? (
    <Circle position='static' marginTop={50} Load={loading} />
   ) : (
    <StudentList width={width}>
     <StudentListItem>
      <StudentListItemInnerWrapper>
       {TopRows.map(item => {
        return !item ? (
         <InnerTopSpan
          style={{flex: "0.06", maxWidth: "29px", minWidth: "29px"}}
          key={item}
         ></InnerTopSpan>
        ) : (
         <InnerTopSpan key={item}>{item}</InnerTopSpan>
        );
       })}
      </StudentListItemInnerWrapper>
     </StudentListItem>
     ...
     {data.length <= 0 && !notFound ? (
      ...
      >
       {" "}
       <i className='fas fa-search'></i> Lütfen Bir Arama Yapınız.
      </h1>
     ) : null}
     ...
     {data
      .map((mainItem, index) => {
       return (
        <StudentListItem
         style={{background: index % 2 == 0 ? "#ececec" : "#fcf8f3"}}
         key={mainItem._id}
         ...
         <StudentListItemInnerWrapper style={{backgroundColor: "#204051"}}>
          <div style={{display: "flex", flex: 1}}>
           {filterIconOptions(mainItem).map((item, Mainindex) => {
            return (
             <StudentListIconWrapper key={item.desc}>
              <Link
               to={pathGenerator(item.desc, mainItem._id)}
               style={{
                display: "flex",
                width: "100%",
                height: "100%",
   ...
   <SubPagesContainer>
    {subPageNumber > 1 &&
     ...
     !notFound &&
     !loading &&
     new Array(subPageNumber).fill().map((item, index) => {
      return (
       <SubPageItem key={index} selected={index === selectedSubPage}>
        <Link
         className='responsiveLink'
         to={
          pathName +
          (Object.keys(oldQueryObject).length <= 1
           ? `?pageNumber=${index + 1}`
           : oldLoactionSearch
              .split("&")
              .slice(0, oldLoactionSearch.split("&").length - 1)
              .join("&") + `&pageNumber=${index + 1}`)
         }
         style={{
          display: "flex",
          alignItems: "center",
          ...
          height: "100%",
          textDecoration: "none",
          color: index === selectedSubPage ? "white" : "grey",
         }}
        >
         {index + 1}
        </Link>
