...
import {
 ...
 makePaymentScheduleRequest,
} from "../../../request/requset";
...
import {
 ...
 confirmStudent as ConfirmStudent,
 IconsendToRosedale,
} from "../../../UI/IconButtons/IconButton";
...
const FileList = styled.ul`
 ...
 position: relative;
 padding: ${({isAllowed}) =>
  isAllowed ? "55px 15px 57px  15px" : "50px 15px 15px  15px"};
 box-shadow: 0 1px 6px -1px rgba(0, 0, 0, 0.4),
  0 2px 4px -1px rgba(0, 0, 0, 0.02);
 margin: 30px 50px;
 border-radius: 15px;
...
const FileListElement = styled.li`
 ...
 position: relative;
 box-shadow: ${({isDeleted}) =>
  isDeleted
   ? "0 1px 6px -1px rgba(0, 0, 0,0.4), 0 2px 4px -1px rgba(0, 0, 0, 0.02)"
   : "0"};
 display: flex;
 justify-content: space-between;
...
const StudentInformations = ({match, ...rest}) => {
 ...
 if (user.role === "Admin") {
  if (student.registerState)
   var updatePermissionResult = user.permissions.Öğrenci_Bilgileri.includes(
    PermissionsNumbers.UPDATE
   );
 } else {
  if (student.registerState)
   var updatePermissionResult =
 ...
 const reduceInstallmentCount = e => {
  ...
  for (let i = 0; i < count; i++) {
   copyPayment.push(
    paymentSchedule[i] ||
     student.paymentSchedule[i] || {amount: null, date: null}
   );
  }
 ...
 if (paymentType) {
  var paymentTable = paymentSchedule.map((item, index) => {
   return (
    <FileListElement
     key={index}
     style={{
    ...
    >
     {width < 1030 ? null : (
      ...
      >
       {paymentType == "Taksit" && `${index + 1}.`} {paymentType}{" "}
      </span>
     )}
 ...
 return loading ? (
  ...
  <MainWrapper>
   <BackStage
    backStage={modalType || backStageLoading}
    close={closeModal}
   ...
   <Title>
    {" "}
    <i style={{marginRight: 8}} class='fas fa-file-alt'></i> Ön Kayıt Bilgileri{" "}
   </Title>
   ...
   <InnerItems>
    ...
    {user.role === "Admin" && getStatusUI.text === "Onay Bekleniyor" ? (
     <ConfirmStudent handler={() => setModalType("CONFİRM_STUDENT")} />
    ) : null}
    ...
    {updatePermissionResult && <IconEdit handler={() => setDisabled(false)} />}

    {user.permissions.Öğrenci_Bilgileri.includes(PermissionsNumbers.REMOVE) && (
     <IconRemove
      handler={() => setModalType("DELETE_STUDENT")}
      deletedText='Öğrenciyi Sil'
     />
    )}
   </InnerItems>

   <FileList
    style={{padding: "65px 15px 40px  15px"}}
    isActive={
     getStatusUI.text === "Sözleşme Yapıldı" ||
   ...
   >
    ...
    {getStatusUI.text === "Öğrenci Onaylandı" && (
     ...
     >
      <TextField
       style={{minWidth: "30%", padding: "10px 0", marginRight: 15}}
       onChange={setPaymentTypeFunc}
       id='select'
       label='Ödeme Şekli'
       value={paymentType}
       select
      ...
      {paymentType === "Taksit" && (
       <TextField
        ...
        style={{minWidth: "30%"}}
        label='Taksit Sayısı'
        value={paymentSchedule.length.toString()}
        onChange={reduceInstallmentCount}
       />
      )}
     </FileListElement>
    )}
    ...
    {getStatusUI.text === "Öğrenci Onaylandı" && (
     <Submit
      ...
      uploadFileLoading={paymentRequest}
      type='button'
      onClick={paymentScheduleRequest}
     >
      {paymentRequest ? (
       <Circle
        Load
       ...
       <React.Fragment>
        Sözleşmeyi Tamamla{" "}
        <i style={{marginLeft: 7}} className='fas fa-file-signature'></i>
       </React.Fragment>
      )}
     </Submit>
    )}
   ...
   <FileList
    isAllowed={uploadFilePermission}
    isActive={student.registerState.docState}
   >
    <FileListElement
     style={{
      background: "#00909e",
   ...
   <RegisterForm
    ...
    id={id}
    setBackStageLoading={setBackStageLoading}
