...
import {
 makeUpdateReportRequest,
 makeDeleteReportRequest,
 makeSpecificReportRequest,
 makeSendFormRequest,
...
const determineStep = initalReportStates => {
 return () => {
  if (initalReportStates.isContacted && !initalReportStates.isFormSent) {
   ...
   initalReportStates.isFormSent &&
   !initalReportStates.isFormFilled
  ) {
   return 2;
...
const ReportDetail = ({match, ...rest}) => {
 ...
 const activeStep = useMemo(determineStep(initalReportStates), [
 ...
 const submitUpdatedReport = e => {
  ...
  let isEmailCorrect = validator.validate(
   initalReportStates["relatedPersonEmail"]
  );
 ...
 return loading ? (
  ...
  <MainWrapper>
   <Stage
    ...
    loading={
     !emptyWarning && !uptadedModal && !deleteModal && !deleted && !sendForm
    }
    close={
     emptyWarning
      ? closeModal_1
   ...
   <Modal
    backStage={sendForm}
    formSent={formSent}
    closeModal={closeModal_5}
    type='SEND_STUDENT_FORM'
   ...
   <Form onSubmit={submitUpdatedReport}>
    {(user.role === "Admin" || user.role === "Temsilci") && (
     <InnerItems
      style={{
     ...
     >
      {initalReportStates.owner &&
       ...
       initalReportStates.owner._id != user._id && (
        <IconForTextLink
         content={`Görüşen Kişi :  ${restrictWord(
          initalReportStates.owner.firstName +
           " " +
           initalReportStates.owner.lastName,
          16
         )}`}
    ...
    <Report
     ...
     isPhoneNumberFilled={checkPhoneNumber(
      initalReportStates["relatedPersonPhoneNumber"]
     )}
    ...
    {disable ? null : (
     <ButtonWrapper>
      <SubmitButton
       ...
       style={{background: "#fa744f", marginRight: 10}}
      >
       VAZGEÇ
      </SubmitButton>
