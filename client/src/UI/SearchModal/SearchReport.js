 ...
 ({
  ...
  useEffect(() => {
   if (Object.keys(queryObject).length > 1) {
    setSearchData(prevState => ({...prevState, ...querySearchData}));
   }
  }, []);
  ...
  const searchSubmitHandler = e => {
   ...
   for (const key in searchMainData) {
    if (
     searchMainData[key] &&
     key !== "dateIntervalStart" &&
     key !== "dateIntervalEnd"
    )
     searchMainData[key] = searchMainData[key].trim();
   }
  ...
  return isOpen ? (
   <SearchModalContainer onSubmit={searchSubmitHandler}>
