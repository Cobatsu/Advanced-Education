...
module.exports.add = async (req, res, next) => {
 try {
  ...
  if (
   updatedReport.owner._id != tokenData.owner ||
   !updatedReport ||
   updatedReport.isFormFilled
  ) {
  ...
  const signedToken = await jwt.sign(
   {studentID: saved._id, owner: tokenData.owner, e_mail: tokenData.e_mail},
  ...
  var mailOptions = {
   ...
   html: `<a href =${
    `${MAIN_PORT}upload?token=` + signedToken
   } > Rosedale Gerekli Belgeleri Yüklemek İçin Lütfen Tıklayınız. </a>`,
  };

  setAuthMail().sendMail(mailOptions, function (error, info) {
   if (error) {
  ...
  if (checkedStudent && !checkedStudent.registerState.docState) {
   if (tokenData.owner == checkedStudent.owner) {
    ...
    await Student.updateOne(
     {_id: tokenData.studentID},
     {$set: {"StudentInfo.Images": images, "registerState.docState": true}}
    );
 ...
 for (const key in body) {
  ...
  if (element) {
   if (key === "gender" || key === "docState") {
    ...
    searchData = {
     ...searchData,
     registerdate: {...searchData.meetingDate, $lte: element},
    };
   } else {
...
module.exports.studentSearch = async (req, res, next) => {
 const {
  user,
  body: {pageNumber, personelReportID, role, ...rest},
 } = req;
 ...
 try {
  ...
  var sortedData = await Student.find(finalSearchQuery)
   .populate("owner")
   .sort({registerdate: "descending"})
   .skip(10 * pageNumber)
   .limit(10);
  ...
  copyReport.forEach((mainItem, index) => {
   ...
   let updatedArray = dateArray.map(item =>
...
module.exports.confirmStudent = async (req, res) => {
 ...
 try {
  await Student.updateOne(
   ...
   {
    $set: {
     "registerState.result.result": true,
     "registerState.pendingResult": false,
    },
   }
  );
...
module.exports.sendConfirmation = async (req, res) => {
 ...
 try {
  await Student.updateOne(
   {_id: id},
   {$set: {"registerState.pendingResult": true}}
  );
...
module.exports.addPaymentSchedule = async (req, res) => {
 const {
  params: {id},
  body: {paymentSchedule, paymentType},
 } = req;
  ...
  await Student.updateOne(
   {_id: id},
   {
    $set: {
     ...
     "registerState.isContracted": true,
    },
   }
  );
...
module.exports.deleteFile = async (req, res) => {
 ...
 try {
  await Student.updateOne(
   {_id: id},
   {$pull: {"StudentInfo.Images": fileWillBeDeleted}}
  );
...
module.exports.uploadFile = async (req, res) => {
 ...
 try {
  await Student.updateOne(
   {_id: id},
   {$push: {"StudentInfo.Images": req.files[0].filename}}
  );
...
module.exports.addNewStudent = async (req, res, next) => {
 ...
 try {
  const newStudent = new Student({
   ...
   isFilled: false,
   owner: user._id,
   allowedToSee: user.relatedAgencyID
    ? [user._id, user.relatedAgencyID]
    : [user._id],
  });
...
module.exports.updateNewAddedStudent = async (req, res, next) => {
 try {
  ...
  const willBeUpdated = await Student.findOne({
   _id: tokenPayload.stundentID,
  }).populate("owner");

  if (
   willBeUpdated.owner._id == tokenPayload.owner &&
   willBeUpdated &&
   tokenPayload.studentMail == willBeUpdated.StudentInfo.e_mail &&
   !willBeUpdated.isFilled
  ) {
   await Student.updateOne(
    {_id: tokenPayload.stundentID},
    {
     $set: {
      StudentInfo: {
       ...studentInformations,
       Images: willBeUpdated.StudentInfo.Images || [],
      },
     },
     isFilled: true,
    }
  ...
  const signedToken = await jwt.sign(
   {
    studentID: tokenPayload.stundentID,
    owner: tokenPayload.owner,
    e_mail: tokenPayload.studentMail,
   },
   process.env.SECRET_KEY
  );
...
module.exports.sendForm = async (req, res, next) => {
 ...
 const tokenLink =
  `${MAIN_PORT}student_form?token=` + token + `&origin=REGISTER`;
