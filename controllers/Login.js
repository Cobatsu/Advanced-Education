    ...
    async function (err, result) {
     if (result) {
      ...
      for (let i = 0; i < Object.keys(rest[type]).length; i++) {
       if (
        !Object.keys(user.permissions).includes(Object.keys(rest[type])[i])
       ) {
      ...
      if (Object.keys(updatedPermissions).length > 0)
       await User.updateOne(
        {_id: user._id},
        {$set: {permissions: {...user.permissions, ...updatedPermissions}}}
       );
      ...
      const token = await jwt.sign(
       {
        ...
        role: user.role,
        relatedAgencyID: user.relatedAgencyID,
       },
