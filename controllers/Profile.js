...
module.exports.addReport = async (req, res, next) => {
 ...
 try {
  const newReport = new Reports({
   ...
   owner: user._id,
   isContacted: true,
   allowedToSee: content,
   reportType,
   whoseDocument: user.firstName + " " + user.lastName,
  });
...
module.exports.addContactReport = async (req, res, next) => {
 ...
 //allowedToSee:content,reportType:'studentReport'
 try {
  const newReport = new Reports({
   ...contactData,
  });
 ...
 for (const key in body) {
  ...
  if (element) {
   if (key === "reportType" || key === "relatedAgencyID") {
    ...
    searchData = {
     ...searchData,
     meetingDate: {...searchData.meetingDate, $lte: element},
    };
   } else {
...
module.exports.reportSearch = async (req, res, next) => {
 ...
 try {
  ...
  var sortedData = await Reports.find(searchData)
...
module.exports.getSpecificReport = async (req, res, next) => {
 ...
 try {
  const specificReport = await Reports.findOne({_id: id})
   .populate("owner")
   .select(" -__v -_id");
...
module.exports.updateReport = async (req, res, next) => {
 ...
 try {
  ...
  var updatedData = await Reports.findOne({_id: id})
   .populate("owner")
   .select("-__v -_id");
